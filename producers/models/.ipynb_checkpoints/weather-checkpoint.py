...
class Weather(Producer):
    ...
    def __init__(self, month):
        ...
        #
        super().__init__(
            "org.chicago.cta.weather", # Done: Come up with a better topic name
    ...
    def run(self, month):
        ...
        resp = requests.post(
            ...
            data=json.dumps(
                {
                    "value_schema": f'{Weather.value_schema}'.replace("'", '"'),
                    "key_schema": f"{Weather.key_schema}".replace("'", '"'), 
                    "records": [
        ...
        try:
            ...
            logger.debug(f"Failed to send data to REST Proxy {json.dumps(resp.json(), indent=2)}")
