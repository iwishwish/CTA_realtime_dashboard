...
class Line:
    ...
    def process_message(self, message):
        ...
        # Half Done: Based on the message topic, call the appropriate handler.
        topic_name = message.topic()
        if "org.chicago.cta.stations.table.faust"==topic_name: # Set the conditional correctly to the stations Faust Table
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif "org.chicago.cta.station.arrivals"==topic_name: # Set the conditional to the arrival topic
            self._handle_arrival(message)
        elif "TURNSTILE_SUMMARY"==topic_name: # Set the conditional to the KSQL Turnstile Summary Topic
